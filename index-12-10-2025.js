...
app.post("/api/productos", autenticarToken, async (req, res) => {
  try {
    ...
    await producto.save();
    const populated = await Producto.findById(producto._id).populate("categoria", "nombre");
...
app.put("/api/productos/:id", autenticarToken, async (req, res) => {
  try {
...
app.delete("/api/productos/:id", autenticarToken, async (req, res) => {
  try {
    const producto = await Producto.findOneAndDelete({ id_producto: req.params.id });
    if (!producto) return res.status(404).json({ mensaje: "Producto no encontrado" });
...
app.post("/api/categorias", autenticarToken, async (req, res) => {
  try {
    ...
    const categoria = new Categoria({ id_categoria: id, ...req.body });
...
app.delete("/api/categorias/:id", autenticarToken, async (req, res) => {
  try {
    const categoria = await Categoria.findOneAndDelete({ id_categoria: req.params.id });
    if (!categoria) return res.status(404).json({ mensaje: "Categoría no encontrada" });
